package nl.esfokk.hexslates.registry
...
typealias RegistrarEntry<T> = HexslatesRegistrar<T>.Entry<out T>

abstract class HexslatesRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Hexslates.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexslatesRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
