package nl.esfokk.hexslates.config
...
object HexslatesClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HexslatesServerConfig.holder.config = HexslatesServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HexslatesServerConfig.onSyncConfig(null)
        }
    }

    @Config(name = Hexslates.MODID)
    class GlobalConfig : GlobalData() {
        @Category("server")
        @TransitiveObject
        val client = ClientConfig()

        // this should only be used inside of this class; use HexslatesServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HexslatesServerConfig.ServerConfig()
    }
